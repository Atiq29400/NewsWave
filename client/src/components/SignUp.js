...
const SignUpForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (errorMessage !== "") {
      ...
      let profession = formData.profession;
      let password = formData.password;

      // Send a request to the server to check if the email is already used
      axios.post('http://localhost:3001/createUser', { fullName, email, profession, password })
        .then(result => {
        ...
        .catch(err => {
          if (err.response.status === 409) {
            alert("This eamil is already in use. Please choose a different email.")
            errorDiv.innerText = 'This email is already in use. Please choose a different email.';
            errorDiv.style.color = 'red'
          }
        });
        ...
        <div className="mb-3">
          ...
          <div className="form-check">
            <input
              ...
              onChange={handleInputChange}
            />
            <label htmlFor='25' className="form-check-label">19-25</label>
          </div>
          <div className="form-check">
            <input
              className="form-check-input"
